...
public class PersonDAOImpl implements IPersonDAO {
	...
	public void insertData() {
		...
		try (Session ses=HibernateUtil.getSession()){
			//get CHild obj
			List<BankAccount> accounts =List.of(new BankAccount("CURRENT", "SBI", 110000.0),
																			new BankAccount("CURRENT", "SBH", 115550.0));
			Person per1=new Person("ROCKEY", "SPJ", accounts);
	...
	public void loadData() {
		try (Session ses=HibernateUtil.getSession()){
			...
			list.forEach(per->{
				...
				List<BankAccount> child=per.getAccounts();
